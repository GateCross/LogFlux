/**
 * Security 误报反馈草稿 / 排除项候选纯逻辑
 * （移植自旧 Vue 版 `src/views/security/policy-feedback-draft.ts`）。
 *
 * 移植说明（task 11.1 / Req 13.7、17.1）：
 *  - 框架无关纯函数，无任何外部依赖，逐字保留原有逻辑语义。
 *  - 保留原模块自含的类型定义（`ExclusionCandidateRemoveType` / `ExclusionCandidate`）
 *    与分隔符常量，保证候选键往返（build/parse）行为与 Vue 版完全一致
 *    （支撑 Property 14：排除项候选键往返一致）。
 */

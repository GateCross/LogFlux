/**
 * Security 策略展示工具纯逻辑（移植自旧 Vue 版 `src/views/security/security-policy-utils.ts`）。
 *
 * 移植说明（task 11.1 / Req 13.7、17.1）：
 *  - 框架无关纯函数，无 Vue / Naive UI / 运行时耦合，逐字保留原有逻辑语义。
 *  - 仅将类型导入从旧 `@/service/api/caddy-policy` 改为本目录共享类型 `./types`。
 *  - 标签字符串与兜底值（`-` 或原值）保持不变，保证相同取值产生与 Vue 版相同的展示标签
 *    （支撑 Property 13：Security 字段映射等价性）。
 */
import type {

...
import { Modal, message, Button, Tag } from 'ant-design-vue';
// DataTableColumns removed - using ant-design-vue Table
import {
  getCaddyConfigApiHistory,
  getCaddyConfigApiHistoryDetail,
  rollbackCaddyConfigApi
} from '#/api/caddy/server';
import { formatCaddyfile } from '../caddy-config-utils';
import type { CaddyConfigHistoryItem } from '#/api/caddy/server';
...
export function useCaddyConfigHistory(opts: {
  ...
  async function fetchHistory() {
    ...
    const { data, error } = await getCaddyConfigApiHistory(opts.currentServerId.value, {
  ...
  async function fetchHistoryDetail(historyId: number) {
    ...
    const { data, error } = await getCaddyConfigApiHistoryDetail(opts.currentServerId.value, historyId);
    if (error) {
  ...
  async function handleRollback(historyId: number) {
    ...
    dialog.warning({
      ...
      onPositiveClick: async () => {
        const { error } = await rollbackCaddyConfigApi(opts.currentServerId.value!, historyId);
        if (error) {
          message.error('回滚失败');
          return;
        }

...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      ...
      <Button size="small" type="primary" @click="addRoute">新增路由</Button>
    </div>
    <Empty v-if="routes.length === 0" description="暂无路由" />
    <VueDraggable v-else v-model="routes" item-key="id" :animation="150" handle=".drag-handle">
      <div v-for="route in routes" :key="route.id" class="border border-gray-200 rounded p-3" :data-route-id="route.id">
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2">
            <Switch v-model:value="route.enabled" size="small" />
            <Button size="tiny" @click="toggleRoute(route.id)">
        ...
        <NCollapseTransition :show="isExpanded(route.id)">
          <div class="mt-3 flex flex-col gap-3">
            <Input v-model:value="route.name" placeholder="路由名称" />
            <div class="border border-gray-200 rounded-md p-3">
              <div class="mb-2 font-medium">Matchers</div>
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <div class="mb-1 text-xs text-gray-500">Host</div>
                  <Select mode="tags" v-model:value="route.match.host" placeholder="输入后回车添加" />
                ...
                <div>
                  ...
                  <Select mode="tags" v-model:value="route.match.path" placeholder="输入后回车添加" />
                ...
                <div>
                  ...
                  <Select mode="tags" v-model:value="route.match.method" placeholder="输入后回车添加" />
                ...
                <div>
                  ...
                  <Input v-model:value="route.match.expression" placeholder="expression {expr}" />
                </div>
              </div>
                  ...
                  <NDynamicInput v-model:value="route.match.header" :on-create="createKeyValue">
                    <template #default="{ value }">
                      <div class="w-full flex gap-2">
                        <Input v-model:value="value.key" placeholder="Key" />
                        <Input v-model:value="value.value" placeholder="Value" />
                      </div>
                    </template>
                  </NDynamicInput>
                </div>
            ...
            <div class="border border-gray-200 rounded-md p-3">
              <div class="mb-2 flex items-center justify-between">
                ...
                <Button size="tiny" @click="addHandle(route.id)">新增 Handler</Button>
              </div>
              <VueDraggable v-model="route.handles" item-key="id" :animation="150" handle=".handle-drag">
                <div v-for="handle in route.handles" :key="handle.id" class="border rounded p-2">
                  <div class="flex items-center gap-2">
                    ...
                    />
                    <Switch v-model:value="handle.enabled" size="small" />
                    <Button size="tiny" type="error" @click="removeHandle(route.id, handle.id)">删除</Button>
                  </div>
                  ...
                  <div class="mt-2">
                    <template v-if="handle.type === 'reverse_proxy'">
                      ...
                      />
                      <div class="mt-2">
                        <Switch v-model:value="handle.tlsInsecureSkipVerify" size="small" />
                        <span class="ml-2 text-sm">TLS 跳过校验</span>
                      </div>
                    </template>
                    <template v-else-if="handle.type === 'file_server'">
                      <Input v-model:value="handle.root" placeholder="Root 路径" />
                      <div class="mt-2">
                        <Switch v-model:value="handle.browse" size="small" />
                        <span class="ml-2 text-sm">目录浏览</span>
                      </div>
                    </template>
                    <template v-else-if="handle.type === 'respond'">
                      <InputNumber v-model:value="handle.status" placeholder="Status" :min="100" :max="599" />
                      <Input v-model:value="handle.body" placeholder="Body" class="mt-2" />
                    </template>
                    <template v-else-if="handle.type === 'redirect'">
                      <Input v-model:value="handle.to" placeholder="跳转地址" />
                      <NInputNumber
                    ...
                    <template v-else-if="handle.type === 'header'">
                      <NDynamicInput v-model:value="handle.rules" :on-create="createHeaderRule">
                        <template #default="{ value }">
                          <div class="w-full flex gap-2">
                            <Select v-model:value="value.op" :options="headerOpOptions" size="small" class="w-24" />
                            <Input v-model:value="value.key" placeholder="Key" />
                            <Input v-model:value="value.value" placeholder="Value" />
                          </div>
                        </template>
                    ...
                    <template v-else-if="handle.type === 'rewrite'">
                      <Input v-model:value="handle.uri" placeholder="Rewrite URI" />
                    </template>
                  </div>
                </div>
              </VueDraggable>
            ...
            <div class="flex gap-2">
              <Button size="tiny" @click="duplicateRoute(route.id)">复制路由</Button>
              <Button size="tiny" type="error" @click="removeRoute(route.id)">删除路由</Button>
            </div>
          </div>
        </NCollapseTransition>
      </div>

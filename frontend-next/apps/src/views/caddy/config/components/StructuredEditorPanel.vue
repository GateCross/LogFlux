  ...
  >
    ...
    <div class="caddy-main-panel min-h-0 min-w-0 flex flex-col flex-1 gap-3 overflow-auto">
      <div class="flex flex-wrap items-center gap-2">
        <Button size="small" @click="props.onApplyPreset">应用默认模板</Button>
        <Button size="small" @click="props.onImportRawToStructured">从原始配置解析</Button>
        <Button size="small" @click="props.onOpenPreviewModal">预览原始 Caddyfile</Button>
      </div>
      <Alert v-if="!props.structuredAvailable" type="warning" title="结构化配置未加载" class="mb-2">
        当前服务器未保存结构化配置，可通过“从原始配置解析”或“应用默认模板”生成。
      </Alert>
      <Card size="small" :bordered="false" class="bg-white">
        <template #header>全局配置（原样保留）</template>
        <template #header-extra>
          <div class="flex items-center gap-2">
            <Tag v-if="props.globalRawChanged" type="warning" size="small">未保存</Tag>
            <Button size="tiny" @click="props.onToggleGlobalPreview">
              {{ props.globalPreviewExpanded ? '收起' : '展开' }}
            </Button>
            <Button size="tiny" @click="props.onOpenGlobalModal">查看/编辑</Button>
          </div>
        </template>
        <pre
      ...
      <Alert v-if="props.validationErrors.length" type="error" title="配置校验错误" class="mb-2">
      ...
      <Collapse class="mt-2">
        <Collapse.Panel title="上游池管理" name="upstreams">

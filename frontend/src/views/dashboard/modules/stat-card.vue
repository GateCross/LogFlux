...
<template>
  <div
    class="stat-card group relative overflow-hidden rounded-2xl p-5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg"
  ...
  >
    <div class="flex items-start justify-between">
      <div class="flex flex-col gap-1.5">
        ...
        <div class="flex items-baseline gap-1.5">
          <span class="font-number text-3xl font-bold" :style="{ color: safeColor }">{{ data.value }}</span>
          <span v-if="data.unit" class="text-xs text-gray-400">{{ data.unit }}</span>
        </div>
        <div v-if="data.trend" class="flex items-center gap-1 text-xs">
          <Icon
            :icon="trendDir === 'up' ? 'mdi:arrow-up' : 'mdi:arrow-down'"
      ...
      <div
        class="flex h-12 w-12 items-center justify-center rounded-full text-xl transition-transform duration-300 group-hover:scale-110"
        :style="iconWrapStyle"
      >
    ...
    <div
      class="absolute -bottom-6 -right-6 h-28 w-28 rounded-full opacity-[0.04] transition-opacity duration-300 group-hover:opacity-[0.08]"

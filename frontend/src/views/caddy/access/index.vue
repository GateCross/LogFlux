...
import { type IPRegionConfig, fetchIPRegionConfig, updateIPRegionConfig } from '@/service/api/ip-region';
...
async function loadConfig() {
  loading.value = true;
  try {
    const { data, error } = await fetchIPRegionConfig();
    if (!error && data) {
...
async function handleSave() {
  saving.value = true;
  try {
    const { error } = await updateIPRegionConfig({
...
<template>
  <div class="p-4">
    <NCard title="访问控制">
      ...
      <NSpin :show="loading">
        <NForm label-placement="top" class="max-w-lg">
          <NFormItem label="启用 IP 区域限制">
            <NSwitch v-model:value="enabled" />
          </NFormItem>

          <NFormItem label="允许访问的地区">
            <NSelect

...
import { previewCaddyConfig, updateCaddyConfigRaw, updateCaddyConfigStructured } from '@/service/api/caddy';
...
export function useCaddyPublishFlow(opts: {
  ...
  async function saveBlocksConfig() {
    ...
    const { data: preview, error: previewError } = await previewCaddyConfig(opts.currentServerId.value, {
      mode: 'quick',
      config: content,
      modules
    });

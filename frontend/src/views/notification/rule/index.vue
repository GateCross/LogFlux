...
const rules = computed(() => ({
  ...
  channelIds: {
    ...
    trigger: 'change'
  } as unknown as import('naive-ui').FormItemRule
}));
  ...
  { title: () => t('page.notification.rule.eventType'), key: 'eventType' },
  {
    ...
    render(row) {
      return h(
        ...
        { default: () => (row.enabled ? t('page.notification.rule.enabled') : t('page.notification.rule.disabled')) }
      );
    }
  },
...
<template>
  <div class="h-full">
    <NCard :title="$t('page.notification.rule.title')" :bordered="false" class="h-full rounded-2xl shadow-sm">
    ...
    <NModal
      ...
      preset="card"
      :title="modalType === 'add' ? $t('page.notification.rule.add') : $t('page.notification.rule.edit')"
      class="w-700px"
    >
      <NForm ref="formRef" :model="formModel" :rules="rules" label-placement="left" label-width="120">
        <NFormItem :label="$t('page.notification.rule.name')" path="name">
          <NInput v-model:value="formModel.name" :placeholder="$t('page.notification.rule.placeholder.name')" />
        </NFormItem>

        <NRow :gutter="20">
          <NCol :span="12">
            <NFormItem :label="$t('page.notification.rule.ruleType')" path="ruleType">
              <NSelect
                v-model:value="formModel.ruleType"
                :options="ruleTypeOptions"
                :placeholder="$t('page.notification.rule.placeholder.type')"
              />
            </NFormItem>
          </NCol>
          <NCol :span="12">
            <NFormItem :label="$t('page.notification.rule.eventType')" path="eventType">
              <NInput
                v-model:value="formModel.eventType"
                :placeholder="$t('page.notification.rule.placeholder.eventType')"
              />
            </NFormItem>
          </NCol>
        </NRow>

        <NFormItem :label="$t('page.notification.rule.enabled')" path="enabled">
          <NSwitch v-model:value="formModel.enabled" />
        </NFormItem>

        <NFormItem :label="$t('page.notification.rule.condition')" path="condition">
          <NInput
        ...
        <NFormItem :label="$t('page.notification.rule.channels')" path="channelIds">
          <NSelect
        ...
        <NFormItem :label="$t('page.notification.rule.template')" path="template">
          <NSelect
        ...
        <NFormItem :label="$t('page.notification.rule.silence')" path="silenceDuration">
          <NInputNumber v-model:value="formModel.silenceDuration" placeholder="0" />
        </NFormItem>

        <NFormItem :label="$t('page.notification.rule.description')" path="description">
          <NInput
            v-model:value="formModel.description"
            type="textarea"
            :placeholder="$t('page.notification.rule.placeholder.description')"
          />
        </NFormItem>
      </NForm>
      <template #footer>

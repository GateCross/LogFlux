    ...
    <n-card :bordered="false" class="rounded-8px shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div class="mt-1 text-xs text-gray-500">将版本发布与任务审计收束到同一运维域，保留激活、回滚、清理与执行追踪主路径。</div>
        </div>
        <div class="flex gap-2">
          <n-button :type="activeSection === 'release' ? 'primary' : 'default'" @click="navigateToTab('release')">发布管理</n-button>
          <n-button :type="activeSection === 'job' ? 'primary' : 'default'" @click="navigateToTab('job')">任务审计</n-button>
        </div>
      </div>
    </n-card>

    <n-card v-if="activeSection === 'release'" :bordered="false" class="rounded-8px shadow-sm">
      <ReleaseTabContent

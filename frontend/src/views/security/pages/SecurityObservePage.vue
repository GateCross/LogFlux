    ...
    <n-card :bordered="false" class="rounded-8px shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div class="mt-1 text-xs text-gray-500">将效果分析与误报处置分区，降低工具栏密度，同时保持 URL 筛选恢复与批量处理链路。</div>
        </div>
        <div class="flex gap-2">
          <n-button :type="activeView === 'analysis' ? 'primary' : 'default'" @click="setActiveView('analysis')">效果分析</n-button>
          <n-button :type="activeView === 'feedback' ? 'primary' : 'default'" @click="setActiveView('feedback')">误报处置</n-button>
        </div>
      </div>
    </n-card>
    ...
    <template v-if="activeView === 'analysis'">
      ...
      <n-card :bordered="false" class="rounded-8px shadow-sm">
        <div class="mb-3 flex flex-wrap gap-2 items-center">
          <n-select v-model:value="policyStatsQuery.policyId" :options="policyStatsPolicyOptions" clearable placeholder="策略范围" class="w-240px" />
          ...
          <n-input-number
            ...
            class="w-180px"
          />
          <n-input-number v-model:value="policyStatsQuery.topN" :show-button="false" :min="1" :max="50" placeholder="TopN" class="w-120px" />
        ...
        <div class="mt-2 flex flex-wrap gap-2 items-center">
          ...
          <n-tag v-if="policyStatsQuery.host" closable size="small" @close="() => clearPolicyStatsDrillLevel('host')">Host: {{ policyStatsQuery.host }}</n-tag>
          <n-tag v-if="policyStatsQuery.path" closable size="small" type="info" @close="() => clearPolicyStatsDrillLevel('path')">Path: {{ policyStatsQuery.path }}</n-tag>
          <n-tag v-if="policyStatsQuery.method" closable size="small" type="warning" @close="() => clearPolicyStatsDrillLevel('method')">Method: {{ policyStatsQuery.method }}</n-tag>
          <span v-if="!hasPolicyStatsDrillFilters" class="text-xs text-gray-400">-</span>
        </div>
      </n-card>

      <n-card :bordered="false" size="small" class="rounded-8px shadow-sm">

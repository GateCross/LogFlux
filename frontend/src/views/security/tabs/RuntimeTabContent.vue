...
<template>
  <div>
    ...
    <div class="mb-3 flex flex-wrap items-center gap-2">
      <NInput
        v-model:value="policyQuery.name"
        placeholder="按策略名称搜索"
        clearable
        class="w-220px"
        @keyup.enter="fetchPolicies"
      />
      <NButton type="primary" @click="fetchPolicies">
        <template #icon>
          <icon-carbon-search />
        </template>
        查询
      </NButton>
      <NButton @click="resetPolicyQuery">重置</NButton>
      <NButton type="primary" @click="handleAddPolicy">
        <template #icon>
          <icon-ic-round-plus />
        </template>
        新增策略
    ...
    <NCard :bordered="false" size="small" class="mb-3 bg-#fafafc">
    ...
    <NCard :bordered="false" size="small" class="mt-3">
      <div class="mb-2 text-sm font-semibold">
        策略指令预览 {{ policyPreviewPolicyName ? `(${policyPreviewPolicyName})` : '' }}
      </div>

...
func (i *SystemIngestor) startFile(filePath string, sourceType string) bool {
	...
	i.tails[filePath] = t
	i.fileSource[filePath] = shared.NormalizeSourceType(sourceType)
	i.mu.Unlock()
...
func (i *SystemIngestor) startDir(dirPath string, scanIntervalSec int, sourceType string) {
	...
	if watcher, exists := i.dirWatchers[dirPath]; exists {
		if watcher.interval == interval && watcher.source == shared.NormalizeSourceType(sourceType) {
			i.mu.Unlock()
			return
		}
	...
	i.dirWatchers[dirPath] = systemDirWatcher{stopCh: stopCh, interval: interval, source: shared.NormalizeSourceType(sourceType)}
...
func (i *SystemIngestor) IngestLine(filePath string, sourceType string, line string) error {
	...
	if shared.NormalizeSourceType(sourceType) == "caddy_runtime" {
	...
	if entry, ok := parsePlainMainLine(line); ok {
		entry.FilePath = filePath
		entry.Source = shared.NormalizeSourceType(sourceType)
...
func (i *SystemIngestor) save(entry *ingestmodel.SystemLog) error {
	if entry.RawLog == "" {
...
func parseCaddyRuntimeJSON(line string) (*ingestmodel.SystemLog, error) {
	...
	entry.Level = shared.AsString(data["level"])
	entry.Message = shared.AsString(data["msg"])
...
func parsePlainMainLine(line string) (*ingestmodel.SystemLog, bool) {
...
func mergeExtra(entry *ingestmodel.SystemLog, extras map[string]string) {

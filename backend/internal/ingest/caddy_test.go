...
func TestIsInternalCaddyAccess(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "private host",
			entry: model.CaddyLog{Host: "192.168.50.10:80", RemoteIP: "8.8.8.8"},
			want:  true,
		},
		{
			name:  "localhost host",
			entry: model.CaddyLog{Host: "localhost", RemoteIP: "8.8.8.8"},
			want:  true,
		},
		{
			name:  "loopback ipv6 host",
			entry: model.CaddyLog{Host: "[::1]:80", RemoteIP: "8.8.8.8"},
			want:  true,
		},
		{
			name:  "private client ip",
			entry: model.CaddyLog{Host: "example.com", RemoteIP: "172.18.0.2", ClientIP: "10.0.0.2"},
			want:  true,
		},
		{
			name:  "private remote ip without client ip",
			entry: model.CaddyLog{Host: "example.com", RemoteIP: "192.168.50.10"},
			want:  false,
		},
		{
			name:  "public client behind private proxy",
			entry: model.CaddyLog{Host: "example.com", RemoteIP: "172.18.0.2", ClientIP: "8.8.8.8"},
			want:  false,
		},
	}

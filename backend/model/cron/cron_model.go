...
type CronTaskModel interface {
	...
	CreateLog(ctx context.Context, log *CronTaskLog) error
	UpdateLog(ctx context.Context, log *CronTaskLog) error
	FindLogByID(ctx context.Context, id uint) (*CronTaskLog, error)
...
func (m *defaultCronTaskModel) CreateLog(ctx context.Context, log *CronTaskLog) error {
	return m.conn(ctx).Create(log).Error
}

func (m *defaultCronTaskModel) UpdateLog(ctx context.Context, log *CronTaskLog) error {
	return m.conn(ctx).Save(log).Error
}

func (m *defaultCronTaskModel) FindLogByID(ctx context.Context, id uint) (*CronTaskLog, error) {
	var logEntry CronTaskLog
	if err := m.conn(ctx).First(&logEntry, id).Error; err != nil {
		return nil, err
	}
	return &logEntry, nil
}

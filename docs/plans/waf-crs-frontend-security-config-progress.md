...
- P4-B02：误报反馈批量处理闭环（批量选择 + 批量状态流转 + 统一处理备注）。
- P4-B03：误报反馈批量处理接口与审计字段补齐（批量处理人、批量处理时间、影响条数）。

## 未完成任务（后续阶段）

- P4-C01：误报反馈与规则例外联动（从反馈快速生成 `removeById/removeByTag` 建议草稿）。
- P4-C02：策略观测导出扩展（在对比 CSV 中补充 Top Host/Path/Method 维度差异块）。

## 下一步建议

1. 优先落地 P4-B02：先做批量状态流转（pending/confirmed/resolved）与批量备注，直接降低人工处理成本。
2. 紧接推进 P4-B03：补齐批量处理后端接口与审计字段，保证可追踪与可回溯。
